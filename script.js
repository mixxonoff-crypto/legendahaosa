...
// НОВАЯ КВАДРАТИЧНАЯ ФОРМУЛА ОПЫТА ДЛЯ КЛАНА
function getRequiredClanExp(level) {
    return 500 + (level * level * 15);
}
...
function handleMonsterDeath(existingLog) {
    ...
    // СОХРАНЕНИЕ ОПЫТА В СТАТИСТИКУ ВКЛАДА КЛАНА (С НОВОЙ КВАДРАТИЧНОЙ ФОРМУЛОЙ)
    if (activePlayer.clanName) {
        let clan = JSON.parse(localStorage.getItem('rpg_clan_' + activePlayer.clanName));
        if (clan) {
            clan.exp += finalMonsterExp;
            let reqClanExp = getRequiredClanExp(clan.level); // Новая формула
            while (clan.exp >= reqClanExp) {
                clan.exp -= reqClanExp; 
                clan.level += 1; 
                reqClanExp = getRequiredClanExp(clan.level); // Новая формула
            }
...
// Полностью исправленная функция открытия экрана Башни Клана
function openClanTower() {
    if (!activePlayer || !activePlayer.clanName) return;
    const clan = JSON.parse(localStorage.getItem('rpg_clan_' + activePlayer.clanName));
    if (!clan) return;

    // Сначала находим элемент в HTML-документе
    const msgBox = document.getElementById('tower-msg');
    
    // И только теперь безопасно очищаем старые сообщения!
    if (msgBox) {
        msgBox.classList.add('hidden');
        msgBox.innerHTML = "";
    }
