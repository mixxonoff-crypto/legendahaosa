...
// СЛОЖНАЯ ФОРМУЛА: Клан больше не будет качаться слишком быстро
...
function handleMonsterDeath(existingLog) {
    ...
    if (activePlayer.clanName) {
        ...
        if (clan) {
            ...
            // Начисляем финальный опыт монстра в копилку клана
            clan.exp += finalMonsterExp;

            // Рассчитываем уровень клана по новой квадратичной формуле
            ...
            // Записываем полученный опыт в общую таблицу вклада (для статистики)
            ...
            // Если уровень клана повысился, выводим красивое сообщение в лог боя
            if (clanLevelUp) {
...
function updateClanUI() {
    ...
    let reqExp = getRequiredClanExp(clan.level); // Новая квадратичная формула
...
function attackArenaBot() {
    ...
    let fightLog = "";
        ...
        // ИСПРАВЛЕНО: НАЧИСЛЕНИЕ ОПЫТА ОБЩЕМУ КЛАНУ ЗА АРЕНУ И ПЕРЕСЧЕТ УРОВНЯ КЛАНА
        if (activePlayer.clanName && clan) {
            if (clan.exp === undefined) clan.exp = 0;
            if (clan.level === undefined) clan.level = 1;

            // Добавляем опыт в общую копилку клана!
            clan.exp += finalBotExp;

            // Считаем уровень клана по квадратичной формуле
            let reqClanExp = getRequiredClanExp(clan.level);
            let clanLevelUp = false;
            ...
            // Записываем полученный опыт в личную статистику вклада
            if (!clan.stats) clan.stats = {};
            if (!clan.stats[activePlayer.username]) {
                clan.stats[activePlayer.username] = { gold: 0, exp: 0 };
            }
            ...
            // Мгновенно сохраняем изменения клана
            localStorage.setItem('rpg_clan_' + activePlayer.clanName, JSON.stringify(clan));

            // Сообщаем в лог, если апнули уровень клана прямо на Арене
            if (clanLevelUp) {
