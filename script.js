...
const monsterDatabase = [
    ...
    { name: "Дракон", hp: 2800, maxHp: 2800, attack: 140, image: "drakon1.jpg", exp: 320, silver: 180, icon: "🐉" },
...
// Параметры Черного Дракона в качестве рейд-босса
let chaosBossState = {
    isSpawned: false,
    hp: 2500000,
    maxHp: 2500000,
...
setInterval(() => {
    ...
    // Каждую нулевую минуту и секунду часа (например 10:00:00, 11:00:00) воскрешаем босса
...
function updateChaosAbyssUI() {
    ...
    // Сценарий 1: У игрока есть неоткрытый сундук за убийство босса
    ...
    // Сценарий 3: Босс спит / уничтожен
    contentZone.innerHTML = `
...
// Логика ударов по боссу Бездны Хаоса
function attackAbyssBoss() {
    ...
    // Вычисляем урон игрока на основе его характеристик
    ...
    if (chaosBossState.hp <= 0) {
        ...
        // Показываем урон на экране
        showDamageAnimation(playerDmg);
    }
}

// Открытие рандомного сундука золота от 1,000,000 до 15,000,000
function openAbyssChest() {
    if (!activePlayer || !chaosBossState.hasChest) return;

    // Генерируем награду в рамках 1м - 15м
