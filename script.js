...
// Открытие экрана Башни Клана
function openClanTower() {
    if (!activePlayer || !activePlayer.clanName) return;
    const clan = JSON.parse(localStorage.getItem('rpg_clan_' + activePlayer.clanName));
    if (!clan) return;

    // Скрываем и очищаем старые сообщения при входе в Башню Клана
...
// Повышение уровня Алтаря за золото Клана
function upgradeAltar(altarKey) {
    ...
    // Проверка на владельца клана (только лидер может улучшать алтари)
    ...
    // Снимаем золото, повышаем уровень алтаря
    clan.gold -= price;
    clan[altarKey] = currentLvl + 1;

    // Сохраняем данные клана
    localStorage.setItem('rpg_clan_' + activePlayer.clanName, JSON.stringify(clan));

    // Пересчитываем статы персонажа (так как Алтарь Войны мог повыситься)
    calculateStats();
    ...
    // Показываем красивый текст успеха прямо на экране Башни
    if (msgBox) {
...
// Отрисовка характеристик Алтарей в Башне
function updateClanTowerUI() {
    if (!activePlayer || !activePlayer.clanName) return;
    const clan = JSON.parse(localStorage.getItem('rpg_clan_' + activePlayer.clanName));
    if (!clan) return;

    // Выводим золото в казне на экран башни
