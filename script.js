...
function handleMonsterDeath(existingLog) {
    ...
    // СОХРАНЕНИЕ ОПЫТА В СТАТИСТИКУ ВКЛАДА КЛАНА
    if (activePlayer.clanName) {
        let clan = JSON.parse(localStorage.getItem('rpg_clan_' + activePlayer.clanName));
        if (clan) {
...
function createClan() {
    ...
    const clanData = { 
        ...
        altarWisdom: 0,
        altarWealth: 0,
        stats: {} // Инициализируем хранилище статистики вкладов
    };
    clanData.stats[activePlayer.username] = { gold: 0, exp: 0 }; // Добавляем создателя в статы
...
// ЗОЛОТАЯ ШАХТА С УЧЁТОМ АЛТАРЯ БОГАТСТВА (+1 К КЛИКУ ЗА УРОВЕНЬ)
...
function mineGold() {
    ...
    // Считаем бонус от Алтаря Богатства
...
// ВЗНОС СЕРЕБРА В КАЗНУ (КОНВЕРТИРУЕТСЯ В ЗОЛОТО 1К1 И ПИШЕТСЯ В СТАТИСТИКУ)
...
function submitSilverDeposit() {
    ...
    // Перерисовываем таблицу вкладов на лету, если её блок открыт
...
function openChaosAbyssScreen() {
    ...
    if (storedState) {
        ...
        // Динамический расчёт под силу игрока (атака игрока * 15)
...
function attackArenaBot() {
    ...
    if (currentArenaBot.hp <= 0) {
        ...
        // СОХРАНЕНИЕ ОПЫТА В СТАТИСТИКУ ВКЛАДА КЛАНА (ЗА ПОБЕДУ НА АРЕНЕ)
        if (activePlayer.clanName) {
            let clan = JSON.parse(localStorage.getItem('rpg_clan_' + activePlayer.clanName));
            if (clan) {
                if (!clan.stats) clan.stats = {};
        ...
        fightLog = `<div style="color: #4de64d; font-weight: bold;">⚔️ Нанесено урона: ${formatNum(finalPlayerDamage)}. Соперник повержен!</div>
                    <div style="color: #ffae19; font-size: 11px; margin-top: 3px;">+${formatNum(finalBotExp)} Опыт | +${formatNum(currentArenaBot.silver)} Серебро</div>`;
        
        if (levelUpOccurred) {
            fightLog += `<div class="lvl-up-text" style="margin-top: 4px;">⭐ Новый уровень! ⭐</div>`;
        }

        // Перерисовываем таблицу вкладов на лету, если игрок находится на экране клана
        const statsBlock = document.getElementById('clan-stats-block');
        if (statsBlock && !statsBlock.classList.contains('hidden')) {
            renderClanStats();
        }
...
// Исправленная функция открытия экрана Башни Клана
function openClanTower() {
    if (!activePlayer || !activePlayer.clanName) return;
    const clan = JSON.parse(localStorage.getItem('rpg_clan_' + activePlayer.clanName));
    if (!clan) return;

    // Находим блок сообщений (теперь он объявлен правильно!)
...
// Показать / Скрыть блок статистики вкладов в меню клана
...
// Построение таблицы статистики вкладов
function renderClanStats() {
    ...
    // Мягкая инициализация структуры статистики, если она отсутствует
    if (!clan.stats) clan.stats = {};

    // Наша простая база содержит только одного игрока, так как игра оффлайн,
    // но система готова к расширению под список участников.
    ...
    // Кнопка сброса отображается исключительно для Лидера Клана
    if (resetContainer) {
        if (clan.owner === activePlayer.username) {
...
// Сброс статистики вкладов (доступно только Лидеру Клана)
function resetClanStats() {
    ...
    showGameConfirm("Вы действительно хотите обнулить показатели вклада (золота и опыта) всех участников? Это действие необратимо.", function() {
        ...
        // Пересоздаем пустые листы для текущего игрока
        clan.stats[activePlayer.username] = { gold: 0, exp: 0 };
