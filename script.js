...
// =====================================================================
const SUPABASE_URL = "https://nssggbcutmujzorglbue.supabase.co";
const SUPABASE_ANON_KEY = "sb_publishable_ihaK7CMb0KRBvKnMY0gWYw_JdPlxzIx";
...
// ---- АККАУНТЫ: полная синхронизация регистрации/логина/прогресса ----
// БЕЗ ЭТОГО каждый телефон вёл свой отдельный локальный список игроков —
// один и тот же ник можно было "занять" отдельно на каждом устройстве,
// а прогресс персонажа не переносился между телефонами. Теперь Supabase —
// источник истины по аккаунту (ник/пароль/весь прогресс), localStorage —
// быстрый локальный кэш поверх него (для мгновенного входа и офлайна).
let accountPushTimer = null;
function pushFullPlayerToServer() {
    if (!supa || !activePlayer || !activePlayer.username || !currentAuthUserId) return;
    if (accountPushTimer) clearTimeout(accountPushTimer);
    accountPushTimer = setTimeout(() => {
        const { username, userId, ...playerDataToSave } = activePlayer;
        supa.from('accounts').upsert({
            username_lower: username.toLowerCase(),
            username: username,
            full_data: playerDataToSave,
            updated_at: new Date().toISOString()
        }, { onConflict: 'username_lower' }).then(({ error }) => { if (error) console.warn("Sync (account push) error:", error.message); });
    }, 3000);
}

// Проверка занятости ника на сервере (перед регистрацией)
async function fetchAccountFromServer(usernameLower) {
    if (!supa) return null;
    const { data, error } = await supa.from('accounts').select('*').eq('username_lower', usernameLower).maybeSingle();
    if (error) { console.warn("Sync (account fetch) error:", error.message); return null; }
    return data;
}

// Регистрация нового аккаунта на сервере (пароль хранится вместе с записью)
async function pushNewAccountToServer(username, password, playerData) {
    if (!supa) return;
...
async function handleRegister() {
    ...
    // Проверяем занятость ника: сначала на сервере (общая база для всех телефонов),
    // затем локально — так ник нельзя занять дважды на разных устройствах.
    const remoteExisting = await fetchAccountFromServer(usernameKey);
    if (remoteExisting) { showGameAlert("Никнейм занят!"); return; }
    const players = ldbRead(LOCAL_DB_KEYS.players);
    ...
    ldbWrite(LOCAL_DB_KEYS.players, players);
    await pushNewAccountToServer(user, pass, playerData); // регистрируем аккаунт на сервере — доступен с любого телефона
...
async function handleLogin() {
    ...
    // Сначала спрашиваем сервер — он источник истины по аккаунту (нужен, если
    // человек регистрировался/играл на ДРУГОМ телефоне). Если сервер недоступен
    // или аккаунт там не найден — используем чисто локальную запись (офлайн-режим).
    const remoteAccount = await fetchAccountFromServer(usernameKey);
    let record;
    if (remoteAccount) {
        if (remoteAccount.password !== pass) { showGameAlert("Неверный пароль!"); return; }
        // Подтягиваем актуальный прогресс с сервера и кладём его в локальный кэш
        record = { id: 'srv_' + usernameKey, username: remoteAccount.username, password: remoteAccount.password, data: remoteAccount.full_data };
        players[usernameKey] = record;
        ldbWrite(LOCAL_DB_KEYS.players, players);
    } else {
        record = players[usernameKey];
...
function saveData() {
    ...
    pushFullPlayerToServer(); // полный прогресс — чтобы вход с другого телефона видел актуальные данные
}
