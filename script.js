...
// Динамическая цена восстановления всей энергии в зависимости от уровня игрока
function getEnergyPrice(level) { 
    // На 1 уровне цена будет ~100 серебра. 
    // На 461 уровне цена составит ~461,000 серебра (что логично при твоих миллионных балансах!)
    return 100 + (level * level * 2); 
}
...
function calculateStats() {
    ...
    activePlayer.attack = 15 + (activePlayer.level * 1000) + forgeAttack + swordBonus;
    activePlayer.defense = 10 + (activePlayer.level * 500) + forgeDefense + shieldBonus;
    activePlayer.maxHp = 1000 + (activePlayer.level * 100) + armorBonus;
...
function checkMultiLevelUp() {
    ...
    while (activePlayer.exp >= required) { 
        ...
        // ДОБАВЛЯЕМ +100 К МАКСИМАЛЬНОЙ ЭНЕРГИИ ПРИ ПОВЫШЕНИИ УРОВНЯ
...
function handleMonsterDeath(existingLog) {
    ...
    let systemLog = `<div class="log-line-system">⚔️ Победа! Получено: Опыт +${formatNum(currentMonster.exp)} | Серебро +${formatNum(currentMonster.silver)}</div>`;
...
function mineGold() {
    ...
    logBox.innerHTML = `⛏️ Бам! Из жилы вывалено <span style="color:#ffd700; font-weight:bold;">+${formatNum(minedGold)}</span> Золота в клановую казну!`; logBox.className = "log-success"; logBox.classList.remove('hidden');
}
...
// Параметры Черного Дракона в качестве рейд-босса Бездны
let chaosBossState = {
    isSpawned: false,
    hp: 2500000,
    maxHp: 2500000,
...
function openChaosAbyssScreen() {
    ...
    if (storedState) {
        ...
        // Балансируем ХП Дракона Бездны под силу атакующего игрока (атака игрока * 15)
...
// Сохранение состояния босса в локальное хранилище
function saveBossState() {
    localStorage.setItem('rpg_chaos_boss', JSON.stringify(chaosBossState));
}

// Секундный таймер проверки спавна (Каждый час ровно в 00:00)
setInterval(() => {
    const now = new Date();
    const minutes = now.getMinutes();
    const seconds = now.getSeconds();

    // Воскрешаем босса Бездны ровно в начале каждого часа
    if (minutes === 0 && seconds === 0) {
        // При воскрешении перерассчитываем ХП под силу игрока, если он уже залогинен
    ...
    // Обновляем таймер на экране, если игрок находится в Бездне Хаоса
...
// Перерисовка контента локации Бездны Хаоса
function updateChaosAbyssUI() {
    const contentZone = document.getElementById('abyss-content');
    if (!contentZone) return;

    // Сценарий 1: У игрока есть неоткрытый сундук за победу над боссом
    ...
    // Сценарий 2: Босс активен и готов к бою
...
// Открытие сундука Бездны (Рандом золота от 1,000,000 до 15,000,000)
...
// Набор генерации префиксов и ников ботов
...
// Генератор ботов под уровень игрока
function generateArenaBot() {
    ...
    // Генерируем бота с разбросом +-2 уровня от уровня игрока
...
function attackArenaBot() {
    ...
    if (currentArenaBot.hp <= 0) {
        // Победа!
        activePlayer.exp += currentArenaBot.exp;
        ...
        fightLog = `<div style="color: #4de64d; font-weight: bold;">⚔️ Нанесено урона: ${formatNum(finalPlayerDamage)}. Соперник повержен!</div>
                    <div style="color: #ffae19; font-size: 11px; margin-top: 3px;">+${formatNum(currentArenaBot.exp)} Опыт | +${formatNum(currentArenaBot.silver)} Серебро</div>`;
        
        if (levelUpOccurred) {
            fightLog += `<div class="lvl-up-text" style="margin-top: 4px;">⭐ Новый уровень! ⭐</div>`;
        }

        // Автогенерация нового соперника для быстрого бесконечного тапанья
        generateArenaBot();
    } else {
        // Ответный удар бота
