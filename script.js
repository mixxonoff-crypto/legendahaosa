...
function handleRegister() {
    ...
    if (!/^[a-zA-Zа-яА-ЯёЁ0-9_]{3,16}$/.test(user)) { showGameAlert("Никнейм: 3-16 символов (буквы, цифры, _)"); return; }
...
function submitSilverDeposit() {
    ...
    if (activePlayer.silverDepositedToday + amount > 10000) { return showGameAlert(`Лимит! Вы можете внести еще максимум ${formatNum(10000 - activePlayer.silverDepositedToday)} серебра сегодня.`); }
    const clan = JSON.parse(localStorage.getItem('rpg_clan_' + activePlayer.clanName)); if (!clan) return;
...
function openForumCategory(category) {
    if (category === 'main' || category === 'ideas') {
        currentOpenedIdeaId = null; // Сбрасываем открытый топик при переходе в разделы
    }
...
setInterval(() => {
    ...
    if (btn) {
        if (active) {
            ...
            btn.innerHTML = "👑 Превосходство Открыто! 🔥";
        } else {
...
function renderChatMessages() {
    ...
    msgs.forEach(m => {
        ...
        msgRow.innerHTML = `
            ...
            <span onclick="insertNicknameReply('${escapeHTML(m.username)}')" style="color: ${nameColor}; font-weight: bold; cursor: pointer; text-decoration: underline;">
...
function insertNicknameReply(username) {
    ...
    // Очищаем или добавляем к текущему тексту (экранируем спецсимволы regex в нике)
    const escapedName = username.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    input.value = `${username}, ` + input.value.replace(new RegExp(`^${escapedName},\\s*`), "");
    input.focus();
}
            ...
            commentsHtml += `
                <div style="background: #14110d; padding: 8px; border-radius: 4px; margin-bottom: 6px; border-left: 2px solid #ffae19; font-size: 11px; text-align: left;">
                    <div style="display: flex; justify-content: space-between; color: #888; font-size: 10px; margin-bottom: 3px;">
                        <span>👤 <span style="color: #ffae19; font-weight: bold;">${escapeHTML(comment.author)}</span>, ${comment.date}</span>
        ...
        container.innerHTML = `
            ...
            <div class="forum-topic" style="background: #0d0d0d; border-radius: 6px; padding: 15px; text-align: left; border: 1px solid #ffaa00; box-shadow: 0 0 10px rgba(255,170,0,0.05);">
                <div class="forum-topic-meta" style="font-size: 11px; color: #888; display: flex; align-items: center; justify-content: space-between;">
                    <div>👤 <span style="color: #ffae19; font-weight: bold;">${escapeHTML(idea.author)}</span>, ${idea.date} ${pinLabel}</div>
                    ${isAdmin ? `
        ...
        html += `
            <div onclick="openIdeaTopic(${idea.id})" style="background: #0d0d0d; border-radius: 4px; padding: 12px; margin-bottom: 10px; text-align: left; cursor: pointer; border: 1px solid #1c1c1c; ${borderStyle} transition: background 0.2s;">
                ...
                <div style="font-size: 11px; color: #666; margin-top: 5px; display: flex; justify-content: space-between;">
                    <span>👤 ${escapeHTML(idea.author)} • 📅 ${idea.date.split(',')[0]}</span>
...
// 6. Функция добавления комментария
...
// 7. Функция удаления комментария (Admin)
