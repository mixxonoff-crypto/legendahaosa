...
function openForumCategory(category) {
    ...
    if (category === 'main') {
        ...
        breadcrumbs.innerHTML = " / <span style='color: #fff;'>Новости</span>";
    } else if (category === 'rules') {
        rulesSec.classList.remove('hidden');
        breadcrumbs.innerHTML = " / <span style='color: #fff;'>Правила общения</span>";
    }
}
...
function isSupremacyActive() {
    const now = new Date();
    // Переводим текущее время в часовой пояс Киева
    ...
    // Воскресенье = 0, Понедельник = 1, ..., Четверг = 4
    const day = now.toLocaleDateString('en-US', { timeZone: 'Europe/Kyiv', weekday: 'numeric' }); // 4 = Четверг
    const hour = parseInt(timeData.hour);

    return (parseInt(day) === 4 && hour === 19);
}
